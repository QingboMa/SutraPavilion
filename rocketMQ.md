...
---
## 1 技术架构
![image-20221022114032060](asset/rocketMQ/pic/image-20221022114032060.png)
...
![image-20221022114042576](asset/rocketMQ/pic/image-20221022114042576.png)

## 2 部署架构


![image-20221022114055250](asset/rocketMQ/pic/image-20221022114055250.png)
...
1. 1. **Remoting Module：整个Broker的实体，负责处理来自Client端的请求。**
   2. **Client Manager：负责管理客户端(Producer/Consumer)和维护Consumer的Topic订阅信息。**
   3. **Store Service：提供方便简单的API接口处理消息存储到物理硬盘和查询功能。**
   4. **HA Service：高可用服务，提供Master Broker 和 Slave Broker之间的数据同步功能。**
   5. **Index Service：根据特定的Message key对投递到Broker的消息进行索引服务，以提供消息的快速查询。**
...
![image-20221022114451231](asset/rocketMQ/pic/image-20221022114451231.png)
...
![image-20221022114507434](asset/rocketMQ/pic/image-20221022114507434.png)
...
![image-20221022114532756](asset/rocketMQ/pic/image-20221022114532756.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_5.png)

#### 2.4 Reactor多线程设计
RocketMQ的RPC通信采用Netty组件作为底层通信库，同样也遵循了Reactor多线程模型，同时又在这之上做了一些扩展和优化。

![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_6.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_7.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_8.png)

(4) 然后，调用updateProcessQueueTableInRebalance()方法，具体的做法是，先将分配到的消息队列集合（mqSet）与processQueueTable做一个过滤比对。

![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_9.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_10.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_11.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_12.png)
...
![img](https://gitee.com/apache/rocketmq/raw/master/docs/cn/image/rocketmq_design_13.png)
...
![image-20221024151445924](asset/rocketMQ/pic/image-20221024151445924.png)
...
![image-20221024151543339](asset/rocketMQ/pic/image-20221024151543339.png)



异步消息通信模式下，各子系统之间无需强耦合直接连接，调用方只需要将请求转化成异步事件（消息）发送给中间代理，发送成功即可认为该异步链路调用完成，剩下的工作中间代理会负责将事件**可靠**通知到下游的调用系统，确保任务执行完成。该中间代理一般就是消息中间件。
...
![image-20221024152223813](asset/rocketMQ/pic/image-20221024152223813.png)
...
![image-20221024152339004](asset/rocketMQ/pic/image-20221024152339004.png)
...
点对点模型和发布订阅模型各有优势，点对点模型更为简单，而发布订阅模型的扩展性更高。 RocketMQ 使用的传输模型为**发布订阅模型**，因此也具有发布订阅模型的特点。
...
​	RocketMq设计的原则是通过topic隔离业务，不同的业务逻辑应使用不同的topic
...
- ​    RocketMQ可以从任意的位点读取任意数量的消息。RabbitMQ、ActiveMQ没有

RocketMQ默认提供消息的可靠存储，所有发送成功的消息都会被持久化到队列中。

RocketMQ队列模型和Kafka的分区（Partition）模型类似。在消息队列RocketMQ版消息收发模型中，队列属于主题的一部分，虽然所有的消息资源以主题粒度管理，但实际的操作实现是面向队列。例如，生产者指定某个主题，向主题内发送消息，但实际消息发送到该主题下的某个队列中。
...
- 消息中转时做好不可变设计
  ...
  5.x版本消息都是只读的视图，所以不用关心

  5.x之前的消息不可变性没有约束，如果需要中专，需要重新初始化消息
...
- 消费状态：RocketMQ默认提供订阅关系持久化的能力，消费者分组在服务端注册订阅关系后，当消费者离线再次上线后，可以获取里先前的消费进度，并继续消费
...
| ANDOR                   | 逻辑与、逻辑或。可用于组合任意简单的逻辑判断，需要将每个逻辑判断内容放入括号内。 | `a IS NOT NULL AND (a > 100) OR (b IS NULL)`：属性a存在且属性a的值大于100或属性b不存在。 |
...
**生产的顺序性**：`必须是单一的生产者进行串行发送`



![image-20221112170909507](asset/rocketMQ/pic/image-20221112170909507.png)

- 相同消息组的消息按照先后顺序被存储在同一个队列。
- 不同消息组的消息可以混合在同一个队列中，且不保证连续。

消息组1和消息组4的消息混合存储在队列1中，消息队列RocketMQ版保证消息组1中的消息G1-M1、G1-M2、G1-M3是按发送顺序存储，且消息组4的消息G4-M1、G4-M2也是按顺序存储，但消息组1和消息组4中的消息不涉及顺序关系。



**消费的顺序性**:必须要保证**投递顺序**和**有限重试**



​	投递顺序：生产者顺序投递后消费者应当按照一接收一应答的语义处理消息，消费者类型为PushConsumer时，消息队列RocketMQ版保证消息按照存储顺序一条一条投递给消费者，若消费者类型为SimpleConsumer，则消费者有可能一次拉取多		条消息。此时，消息消费的顺序性需要由业务方自行保证
...
Kafka消费失败支持重试，KafkaProducer通过设定参数`retries`
...
[ 幂等性 幂等](https://blog.csdn.net/sugelachao/article/details/124410578)

## 参考文档

[快速开始 | RocketMQ (apache.org)](https://rocketmq.apache.org/zh/docs/quickStart/02quickstart/)

[docs/cn · Apache/RocketMQ - 码云 - 开源中国 (gitee.com)](https://gitee.com/apache/rocketmq/tree/master/docs/cn)

[RocketMQ5.0 四大核心特性。 - 教程文章 - 时代Java，与您同行！ (nowjava.com)](https://nowjava.com/article/44240)